...
%%%%%5
vg1list = [4.4 4.4 4.4 4.4];
vglist = [4*ones(size(vg1list))];
vplist = [4.72*ones(size(vg1list))];
vp1list = [6.31*ones(size(vg1list))];
a2list= [0.2 0.4 0.6 0.8];
periodlist = [125*ones(size(vg1list))];
...
for x = xlist
    ...
    [A_xt] = makegaussianwavepacket_FirstOrderDispersion(1,sigma,vg,tlist,period,x,vp);
        [A_xt_2] = makegaussianwavepacket_FirstOrderDispersion(a2,sigma,vg1,tlist,period,x,vp1);
...
plot(degs,GrpTime-GrpTime_n0,'-o','linewidth',2,'color','m')
hold on
ylim([-100 100])
xlim([5 150])
...
grid on; box on;
title(['A_0 = ' num2str(1) ', A_1 = ' num2str(a2)])



end
xlabel('Epicentral Distance (deg)')
sgtitle('Varying Overtone Amplitude','fontweight','bold')
set(gcf,'position',[37 54 319 809])
saveas(gcf,'VaryingA.eps','epsc')
